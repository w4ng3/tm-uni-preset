<template>
  <view
    @mouseleave="touchsend"
    @touchcancel="touchsend"
    ref="tmspin"
...
function touchstart(event: TouchEvent | MouseEvent) {
  ...
  if (event.type.indexOf("mouse") == -1 && event.changedTouches.length == 1) {
    ...
    drawhd.down(
      event.pageX - event.currentTarget.offsetLeft - ctxLeft,
      event.pageY - event.currentTarget.offsetTop - ctxTop
    );
  }
}
...
function save(): Promise<string> {
  return new Promise((su, fa) => {
    ...
    ctx.toTempFilePath(
      ...
      props.width,
      props.height,
      uni.upx2px(props.width),
      uni.upx2px(props.height),
      "png",
      1,
      function (res) {
        uni.hideLoading();
